...
export const generateColors = () => {
  ...
  const parse = (obj) => {
    for (let key in obj) {
      scss.push(`$${key}: ${obj[key]};`);
    }
  };

  // Checking basic possible errors
  if (options.wallpaper.value == "") {
    console.warn("Wallpaper is null, using default colors...");
  ...
  if (!which("matugen")) {
    console.error("Matugen not installed. Using default colors...");
    Utils.exec(["cp", `${OUT}.fall`, OUT]);
  }
  ...
  let scss = [
    "// Generated by ags (generateColors() style.ts) (xtremethn dotfiles), don't edit manually.\n",
  ];

  if (options.dark_mode.value) {
    parse(out.colors.dark);
  } else {
    parse(out.colors.light);
  }
